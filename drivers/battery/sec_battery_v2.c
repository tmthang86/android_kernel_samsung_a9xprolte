	...
	if (battery->cable_type == POWER_SUPPLY_TYPE_BATTERY) {
		...
		if (battery->current_event & SEC_BAT_CURRENT_EVENT_AFC) {
			if (battery->cable_type == POWER_SUPPLY_TYPE_WIRELESS ||
				...
				input_current = (input_current > battery->pdata->pre_afc_input_current) ?
					battery->pdata->pre_afc_input_current : input_current; // 1000mA
				wake_lock(&battery->afc_wake_lock);
	...
	struct sec_battery_info *battery)
{
...
#if defined(CONFIG_STEP_CHARGING)
	if (battery->step_charging_status < battery->pdata->chg_high_temp_start_step)
		return;
#endif
	if (battery->siop_level >= 100 && battery->pdata->chg_temp_check &&
		...
		//dual-charger model . limit the  output current
		if ((battery->chg_limit == SEC_BATTERY_CHG_TEMP_NONE) &&
			...
			if (battery->charging_current > battery->pdata->chg_charging_limit_current) {
			...
			if (battery->charging_current > battery->pdata->chg_charging_limit_current_2nd) {
		...
		struct sec_battery_info *battery)
{
	...
	if (pdata->chg_temp_check) {
		...
		ret = of_property_read_u32(np, "battery,chg_high_temp_start_step",
					   &pdata->chg_high_temp_start_step);
		if (ret)
			pr_info("%s : chg_high_temp_start_step is Empty\n", __func__);
	}
