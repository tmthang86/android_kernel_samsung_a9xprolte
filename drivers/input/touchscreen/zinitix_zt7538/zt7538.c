		...
		unsigned long action, void *data)
{
	...
	if (atomic_read(&misc_info->secure_enabled)) {
		pr_info("%s: ignored, because secure mode, old:%d, TA:%d\n", __func__, old_ta_status, ta_connected);
		return 0;
	}else{
		old_ta_status = ta_connected;
	}
#endif
...
/* Enter sleep mode and gesture mode, add retry to make sure enter successfully, or else sleep current too big*/
		while(retry--)
		{
			if(!write_cmd(info->client, ZT7538_SLEEP_CMD))
				break;

			dev_err(&info->client->dev, "Enter sleep mode failed(%d)\n", retry);
			msleep(10);
		}
		enable_irq(info->irq);
