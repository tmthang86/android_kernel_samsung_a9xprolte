...
static void sm5705_get_fromatted_dump(struct regmap_desc *pdesc, char *mesg)
{
	...
	val = i2c_smbus_read_byte_data(muic->i2c, REG_INT1);
	sprintf(mesg+strlen(mesg), "IN1:%x", val);
	val = i2c_smbus_read_byte_data(muic->i2c, REG_INT2);
	sprintf(mesg+strlen(mesg), "IN2:%x", val);
	val = i2c_smbus_read_byte_data(muic->i2c, REG_CHGTYPE);
	sprintf(mesg+strlen(mesg), "CHG:%x", val);
}
